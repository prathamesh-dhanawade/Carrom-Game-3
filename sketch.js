...
var pocketSound,wallSound,strikingCoinSound;
function preload(){

  pocketSound= loadSound("pocket.mp3")

  wallSound= loadSound("wall.mp3")

  strikingCoinSound= loadSound("strikingCoin.mp3")
...
function setup() {
  ...
  pinkCoin = createSprite(270,247,10,30);
...
text("PLAYER BLACK SCORE:"+blackScore,334,493)
  ...
  // 
   pinkCoin.x = -100;
   pinkCoin.y = -100;
   pinkCoin.setSpeedAndDirection(0);
...
bounceOffWithEachOther();
  if(players==="PLAYERW"){
    //stroke ("black")
    textSize(14)
    fill ("white")
    text("CLICK HERE -->", 18,415)

    if(pinkCoin.isTouching(pocket1)||pinkCoin.isTouching(pocket2)||pinkCoin.isTouching(pocket3)||pinkCoin.isTouching(pocket4)){
      whiteScore+=5;
  ...
  if(striker.getSpeed()< 0.2){
    if(mousePressedOver(invisibleGround2)){
      striker.x=mouseX;
      striker.y=mouseY;
  ...
  if(players==="PLAYERB"){
    ...
    textSize(14)
    fill ("white")
    text("CLICK HERE -->", 18,88)

    
    if(striker.getSpeed()< 0.2){
      if(mousePressedOver(invisibleGround1)){
        striker.x = mouseX;
        striker.y = mouseY;
...
function bounceOffWithEachOther(){
